...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    EditorTableViewCell *editorCell =
    (EditorTableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier forIndexPath:indexPath];
    
    [self configureCell:editorCell atIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        ...
        Editor *editorToDelete = (Editor *)[self.fetchedResultsController objectAtIndexPath:indexPath];
        
        if ([editorToDelete.listEdition count] == 0)
        {
...
- (void)controller:(NSFetchedResultsController *)controller didChangeObject:(id)anObject atIndexPath:(NSIndexPath *)indexPath forChangeType:(NSFetchedResultsChangeType)type newIndexPath:(NSIndexPath *)newIndexPath {
    switch (type) {
        ...
        case NSFetchedResultsChangeUpdate: {
            [self configureCell:(EditorTableViewCell *)[self.tableView cellForRowAtIndexPath:indexPath] atIndexPath:indexPath];
            break;
        }
...
- (void)configureCell:(EditorTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    // Fetch Record
    Editor *record = [self.fetchedResultsController objectAtIndexPath:indexPath];
