...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *CellIdentifier = @"LanguageTableViewCell";
    // dequeue a GameTableViewCell, then set its game to the game for the current row
    LanguageTableViewCell *languageCell =
    (LanguageTableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier forIndexPath:indexPath];
    
    [self configureCell:languageCell atIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        ...
        Language *languageToDelete = (Language *)[self.fetchedResultsController objectAtIndexPath:indexPath];
        
        if ([languageToDelete.listEdition count] == 0)
        {
...
- (void)controller:(NSFetchedResultsController *)controller didChangeObject:(id)anObject atIndexPath:(NSIndexPath *)indexPath forChangeType:(NSFetchedResultsChangeType)type newIndexPath:(NSIndexPath *)newIndexPath {
    switch (type) {
        ...
        case NSFetchedResultsChangeUpdate: {
            [self configureCell:(LanguageTableViewCell *)[self.tableView cellForRowAtIndexPath:indexPath] atIndexPath:indexPath];
            break;
        }
...
- (void)configureCell:(LanguageTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    // Fetch Record
    Language *record = [self.fetchedResultsController objectAtIndexPath:indexPath];
