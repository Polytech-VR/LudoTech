...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ThemeTableViewCell *themeCell =
    (ThemeTableViewCell *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier forIndexPath:indexPath];
    
    [self configureCell:themeCell atIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        ...
        Theme *themeToDelete = (Theme *)[self.fetchedResultsController objectAtIndexPath:indexPath];
        
        if ([themeToDelete.listEdition count] == 0)
        {
...
- (void)controller:(NSFetchedResultsController *)controller didChangeObject:(id)anObject atIndexPath:(NSIndexPath *)indexPath forChangeType:(NSFetchedResultsChangeType)type newIndexPath:(NSIndexPath *)newIndexPath {
    switch (type) {
        ...
        case NSFetchedResultsChangeUpdate: {
            [self configureCell:(ThemeTableViewCell *)[self.tableView cellForRowAtIndexPath:indexPath] atIndexPath:indexPath];
            break;
        }
...
- (void)configureCell:(ThemeTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    // Fetch Record
    Theme *record = [self.fetchedResultsController objectAtIndexPath:indexPath];
